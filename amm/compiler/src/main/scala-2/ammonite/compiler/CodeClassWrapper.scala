...
""")
        ...
        .collect {
          case (key, Seq(path)) =>
            ...
            s"val ${key.backticked}: $encoded.type = " +
              s"if (__amm_usedThings($tq${key.raw}$tq)) $encoded else null.asInstanceOf[$encoded.type]$newLine"
          case (key, values) =>
            throw new Exception(
      ...
      val usedThingsSet =
        if (reqVals.isEmpty) ""
          ...
          s"""
@_root_.scala.transient private val __amm_usedThings = _root_.scala.Predef.genericArrayOps(""" +
