...
object AmmonitePlugin{
  ...
  def apply(g: Global)
    ...
    val stats = {
      ...
      (0 until nestingLevel).foldLeft(unit.body.children)((res, _) =>
    ...
    userCodeNestingLevel match {
      ...
      case 2 =>
         ...
         */
        val wrapperSym = unit.body.children.last.asInstanceOf[g.ImplDef].symbol
        val uses0 = for {
          tree <- stats
          names <- tree.collect {
    ...
    val symbols = stats.filter(x => !Option(x.symbol).exists(_.isPrivate))
      ...
      case (ctx, t @ g.Import(expr, selectors)) =>
        ...
        val prefix = {
          ...
          val headFullPath = NameTransformer.decode(symbolHead.fullName).split('.')
          ...
          val rootPrefix = if(symbolHead.isPackage) Seq("_root_") else Nil
          val tailPath = nameListTail.map(_.decoded)

          (rootPrefix ++ headFullPath ++ tailPath).map(Name(_))
        }
        ...
        val renameMap = {
          ...
          val renamings = for{
            ...
            x <- Option(rename)
          } yield ((isType, x.decoded), name.decoded)
        ...
        val symNames = {
          ...
          for {
            ...
            if saneSym(sym)
          } yield (sym.isType, sym.decodedName)
        }
