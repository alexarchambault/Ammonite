...
object Highlighter {
  ...
  def defaultHighlight(parser: P[_] => P[Any],
                       buffer: Array[Char],
                       commentReset: Long,
                       commentApply: Long,
                       typeReset: Long,
    ...
    val comment = fansi.FansiHelper.attr(commentReset, commentApply)
    val `type` = fansi.FansiHelper.attr(typeReset, typeApply)
    val literal = fansi.FansiHelper.attr(literalReset, literalApply)
    val keyword = fansi.FansiHelper.attr(keywordReset, keywordApply)
    val reset = fansi.FansiHelper.attr(resetReset, resetApply)
    defaultHighlight(parser, buffer, comment, `type`, literal, keyword, reset)
  }
  ...
  def defaultHighlight(parser: P[_] => P[Any],
                       ...
                       literal: fansi.Attrs,
                       keyword: fansi.Attrs,
                       reset: fansi.Attrs): String = {
    val boundedIndices =
      defaultHighlightIndices(parser, buffer, comment, `type`, literal, keyword, reset)

    flattenIndices(boundedIndices, buffer)
  }
  def defaultHighlightIndices(parser: P[_] => P[Any],
                              buffer: Array[Char],
                              comment: fansi.Attrs,
                              `type`: fansi.Attrs,
                              literal: fansi.Attrs,
                              keyword: fansi.Attrs,
                              reset: fansi.Attrs) = highlightIndices(
    parser,
    buffer,
    {
      case "Interp" => reset
