...
class Preprocessor(
  ...
  private def parse(source: String): Either[Seq[String], List[untpd.Tree]] = {
    ...
    val parser = new Parser(sourceFile)(using parseCtx)
  ...
  def transformOrNull(
    ...
    imports: Imports,
    printerTemplate: JFunction[String, String],
    extraCode: String,
    skipEmpty: Boolean,
    markScript: Boolean,
  ...
  private val ClassDef = DefProc("class"){ case m: untpd.TypeDef if m.isClassDef && !m.mods.flags.is(Flags.Trait) => m.name }
  private val TraitDef = DefProc("trait"){ case m: untpd.TypeDef if m.isClassDef && m.mods.flags.is(Flags.Trait) => m.name }
  ...
  private def complete(code: String, resultIndex: String, postSplit: Seq[String]): Either[String, Expanded] = {
