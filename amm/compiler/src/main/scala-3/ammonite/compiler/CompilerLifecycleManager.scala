...
class CompilerLifecycleManager extends ammonite.compiler.iface.CompilerLifecycleManager {
  ...
  def userScalaVersion = dotty.tools.dotc.config.Properties.versionNumberString
  ...
  private[this] object Internal{
    val dynamicClasspath = new VirtualDirectory("(memory)")
    var compiler: ammonite.compiler.iface.Compiler = null
    val onCompilerInit = mutable.Buffer.empty[DottyCompiler => Unit]
    val onSettingsInit = mutable.Buffer.empty[ScalaSettings => Unit] // TODO Pass a SettingsState too
